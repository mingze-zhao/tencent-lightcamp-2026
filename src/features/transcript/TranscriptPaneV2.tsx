...
import type { SourceRef, TranscriptSegment } from '@/types';
...
export default function TranscriptPaneV2() {
  const {
    state: { selectedSession, recordingState, recordingSeconds, transcriptSearchKeyword, activeSegmentId, activeSourceRefId, settings },
    setSearchKeyword,
    setActiveSegment,
    setActiveSourceRef,
    toggleRecording,
        ...
        {menuOpen ? (
          <div className="absolute right-6 top-12 z-10 w-44 rounded-lg border border-slate-200 bg-white p-1 text-sm shadow-lg">
            <button
              ...
              onClick={() => {
                pushToast({ type: 'info', title: 'Export placeholder' });
                setMenuOpen(false);
              }}
            >
              Export
            </button>
            <button className="w-full rounded px-2 py-1.5 text-left hover:bg-slate-100" onClick={() => setSearchKeyword('')}>
              Clear search
            </button>
          </div>
        ) : null}
              ...
              return (
                ...
                >
                  <div className="w-12 flex-shrink-0 pt-1 text-right text-xs text-slate-400">
                    {String(Math.floor(turn.startTime / 60)).padStart(2, '0')}:{String(turn.startTime % 60).padStart(2, '0')}
                  </div>
                      ...
                      {splitByRefs(turn, refsBySegment[turn.id] ?? []).map((piece, pieceIdx) => {
                        ...
                        return (
                          <span
                            ...
                            className={transcriptSearchKeyword.trim() && piece.text.includes(transcriptSearchKeyword.trim()) ? 'rounded bg-blue-100 px-1 py-0.5 text-blue-800' : ''}
                          >
                            {piece.text}
                          </span>
                        );
